...
void render_metrics_panel(struct ncplane* plane, int y, int x, int h, int w, const SolarLunarCoords& coords, double diurnal, double epoch, double horizon, double grand_cyc) {
    ...
    print_row(2, 0xF1FA8C, "ANNUAL  [S]", coords.solar_longitude, coords.solar_longitude / 360.0);
    print_row(3, 0xFFB86C, "EPOCH   [M]", epoch * 360.0, epoch);
    print_row(4, 0xBD93F9, "HORIZON [H]", horizon * 360.0, horizon);
    print_row(5, 0x8BE9FD, "LUNAR-D [☾]", coords.lunar_phase * 360.0, coords.lunar_phase);
    print_row(6, 0xFF79C6, "GRAND-C [G]", grand_cyc * 360.0, grand_cyc);
}

void render_wave_panel(struct ncplane* plane, int y, int x, int h, int w, double diurnal, double lunar_phase) {
    ...
    for (int i = 0; i < max_pts; ++i) {
        ...
        double wave_val = std::sin((norm_i + diurnal) * 2.0 * M_PI) * std::cos((norm_i + lunar_phase) * M_PI);
...
int main() {
    ...
    while (true) {
        ...
        // 3. Chronology Engine Dynamic Vector Computation
        auto vector_coords = ChronologyEngine::computeVector(abs_day);
        ...
        int metrics_h = 8;
        render_metrics_panel(stdplane, metrics_y, 1, metrics_h, dimx - 2, vector_coords, diurnal, epoch, horizon, grand_cyc);

        // 4. Resonance Wave
        int wave_y = metrics_y + metrics_h;
        int wave_h = std::max(4, static_cast<int>(dimy - wave_y - 1));
        render_wave_panel(stdplane, wave_y, 1, wave_h, dimx - 2, diurnal, vector_coords.lunar_phase);

        // Status Bar
        ncplane_set_fg_rgb8(stdplane, 98, 114, 164);
        std::stringstream status_ss;
        status_ss << days_of_week[abs_day % 7] << " | JD: " << std::fixed << std::setprecision(1) << vector_coords.julian_day 
                  << " | ABS: #" << abs_day << " | [W/S/A/D or Arrows]";
        ...
        if (key != 0 && key != (uint32_t)-1) {
            if (key == NCKEY_UP || key == 'w' || key == 'W' || key == 'k' || key == 'K') { abs_day += 1; }
            else if (key == NCKEY_DOWN || key == 's' || key == 'S' || key == 'j' || key == 'J') { abs_day -= 1; }
            else if (key == NCKEY_RIGHT || key == 'd' || key == 'D' || key == 'l' || key == 'L') { abs_day += 30; }
            else if (key == NCKEY_LEFT || key == 'a' || key == 'A' || key == 'h' || key == 'H') { abs_day -= 30; }
            else if (key == NCKEY_PGUP || key == 'u' || key == 'U') { abs_day += 365; }
            else if (key == NCKEY_PGDOWN || key == 'n' || key == 'N') { abs_day -= 365; }
